...
//
// This owns the constraints/flex/grid/scroll math and produces a
// platform-neutral SceneLayout (draw layers plus hit and scroll targets).
// Text measurement is the only platform-specific input, so it is injected as
// a MeasureTextFn callback rather than called directly. Platform shells
// (Core Text on macOS, GDI on Windows) consume the SceneLayout to paint.
...
// shell so this module stays free of any platform text engine.
using MeasureTextFn =
