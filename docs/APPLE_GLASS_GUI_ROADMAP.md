...
| `examples/file_explorer_desktop` | Finder-style desktop app example | Exercises plain toolbar/sidebar/content surfaces with interaction-only glass controls, Finder-like action clusters, document/raster-image/SVG/vector/video/folder thumbnail probes, and sandboxed select/open/read/create/duplicate/delete file workflows |
| `examples/file_explorer_mobile` | Mobile file explorer app example | Exercises a compact browse/preview/create flow with the same sandboxed file model, duplicate/delete action metadata, and plain startup surfaces with interaction-only glass controls |
