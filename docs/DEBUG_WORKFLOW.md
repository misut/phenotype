...
material executor work. They also assert the sampled upload/draw booleans and
`uploaded`/`drawn` status strings so a passing capture cannot hide an edge
status regression behind matching numeric counters.
...
backdrop-copy pixels, and shared capture pixels at zero, with upload/draw status
remaining `not-needed`, while preserving the fallback material plan.
...
flattened metric, value, source path, likely pass, rendered text, source failure
context (`failure_index`, `failure_number`, failure path, and failure
name/message), and total/shown/omitted/limit/truncated counts; the raw source
descriptor remains under `source`. The same structured object is repeated on
each compact `failures[]` detail that has missing-field sources, so automation can consume
...
expectations. The standard manifest pins the sampled-backdrop capture and
...
instead of the full root matrix. The main-branch push workflow only runs docs
builds and native artifact builds; it does not repeat root code tests or slow glass artifact
