...
| `examples/file_explorer_desktop` | macOS, Windows | Finder-style desktop product workflow with glass toolbar, sidebar locations, Recents icon grid, document thumbnails, search, create, delete, and sandboxed temp-root file operations |
...
| `examples/file_explorer_desktop` | Same environment hook with `.exon/debug/file_explorer_desktop`, or `tools/verify_file_explorer_artifacts.sh` from the repo root | Verifies a Finder-style desktop startup scene with glass toolbar/sidebar/icon-grid/status surfaces, stable document thumbnail labels, stable file-operation labels, semantic/runtime material parity, explicit material surface roles, material container identity, and bounded material resource budgets; local gate only by default |
