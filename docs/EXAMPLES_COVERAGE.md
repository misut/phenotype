...
|---|---|---|
| `examples/native` | macOS, Windows | Compact desktop widget showcase for shared controls, input debug, local/remote bitmap images, inline SVG vector image rendering, material-backed dialog sheet overlay, scroll, resize, and manual acceptance |
...
| `examples/file_explorer_desktop` | macOS, Windows | Finder-style desktop product workflow with native integrated chrome, audited macOS-style SVG toolbar/sidebar icons, pure `icons::macos_presentation(...)` role/state mapping, and verifiable Apple-HIG/SF-Symbols-inspired catalog metadata, Finder-style segmented toolbar metrics, split toolbar control material metadata, icon-led column location rows, outline/list row material presets, viewport-aware icon/list/column/gallery panes, plain toolbar/sidebar/content surfaces with hover/press-only glass controls, sidebar locations, real sandboxed Trash, Recents icon grid, document/image/video/folder thumbnails, icon-revealed search, separate select/open activation, file/folder create, duplicate, delete, file preview, and sandboxed temp-root operations |
...
- `examples/file_explorer_desktop` and `examples/file_explorer_mobile` are
  product-style material workflow targets. Their checked-in manifests require
  stable labels, button/material/text-field roles, startup regular material
  surfaces, material surface roles, material plan output, semantic/runtime material parity,
