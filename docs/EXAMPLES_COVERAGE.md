...
artifact/docs build gates, not the full code test matrix. WASI root tests and
docs builds run on Linux runners; macOS runners are reserved for the native
glass artifact gate. Workflow-file changes deliberately enable all relevant
gates so runner policy edits validate themselves.
...
- `examples/file_explorer_desktop` and `examples/file_explorer_mobile` are
  ...
  surface roles, material plan output, semantic/runtime material parity, and
