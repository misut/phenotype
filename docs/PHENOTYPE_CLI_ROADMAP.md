...
as `fail`, `tight`, `pass`, or `unknown` and counts zero/negative margins. The
CLI JSON envelopes also expose the grouped `verifier_bound_pressure` outside
failure summaries whenever a parsed report has bound summaries, so automation can
distinguish a hard budget failure from a passing Liquid Glass budget that has no
headroom left.
