...
| `phenotype drive file-explorer` | implemented | Drives the shared sandboxed desktop/mobile file explorer model from typed CLI inputs or line-based scripts and emits a stable observation JSON with trace, entries, viewport, view mode, pure Finder chrome/grid metrics, default glass theme metadata, capabilities, operation receipt, preview excerpt fields, localized labels, package-resource metadata, and optional expectation results. |
