...
- **Core modules**: types, material.types, material.plan, material, io, theme_contract, state, layout, widget.glass, paint, commands, diag, theme_json
...
```
Callback or binding mutation
  │
  ▼
Component body builds layout tree (Arena + LayoutNode)
  │
  ▼
...
it at state-sync boundary theme refresh, so an OS font, appearance, accent, or scroll
...
`layout::tooltip` and `layout::context_menu` are typed overlay-surface helpers:
tooltips stay passive thin material, while context menus are interactive
regular material. Both still lower through `MaterialSurfaceOptions`, so native
backends execute the same pure plan instead of receiving a new overlay-only
policy branch.
...
edge highlight, bounded refraction, shadow depth, noise/dither, foreground
vibrancy, and deterministic fallback. This gives CI and LLM debugging one
...
- **macOS**: AppKit shell + CoreText text measurement/atlas + Metal renderer + native `DrawImage` for local files and async remote images. The shell creates an `NSWindow`, moves it to the active Space, primes AppKit's WindowServer ordering once, installs a small application delegate so Dock reopen/app-activation events reorder the existing window, passes `NSWindow` / `NSView` plus viewport metadata through `NativeSurfaceDescriptor`, and maps `IntegratedTitlebar` to a transparent full-size titlebar with background dragging so examples can extend content behind native window controls without drawing duplicate chrome. Integrated-titlebar windows also keep the `NSWindow` non-opaque, use a clear AppKit background color, keep the `CAMetalLayer` non-opaque, and clear the drawable with alpha 0 so sidebar glass can reveal the desktop/backdrop instead of flattening to an opaque Metal surface before material planning runs. Finder-style examples keep their root background transparent too; otherwise the app's own full-frame fill would cover the same WindowServer backdrop that AppKit is supplying. The same edge adapter installs an `NSVisualEffectView` as a sibling underlay behind the Metal content view, using behind-window blending; `WindowOptions.native_backdrop_material` selects the system material, defaulting to `under-window-background` while Finder-style sidebars request the AppKit `sidebar` material so transparent pixels reveal the native blurred sidebar backdrop instead of an app-internal flat fill. `WindowOptions.native_backdrop_opacity` configures only that sibling underlay, not the Metal content view, so reducing native backdrop strength cannot fade text, icons, or controls. The expected material, alpha, sibling-underlay requirement, transparent-window requirement, and failure vocabulary come from the pure `plan_native_backdrop_composition()` helper in the shared native platform contract; the AppKit backend supplies live readbacks and executes that plan instead of owning the policy. This is a platform compositor input, not material policy, so the pure `MaterialPlan` still decides in-app glass surfaces while AppKit supplies the native blurred desktop/window backdrop underneath transparent pixels. Runtime artifacts publish live AppKit chrome readbacks, WindowServer bounds, z-order/app-window order, active-Space collection behavior, onscreen state, activation state, key/main-window state, native window/layer transparency fields, visual-effect underlay placement/alpha fields, a `glass_backdrop_composition` readiness summary, clear alpha, and full-frame opaque fill counts to catch regressions where a process owns a Dock icon but the ordered window is hidden, covered, opaque, unblurred, over-transparent, or inaccessible. The Metal renderer executes SVG/icon circles and isolated circular SVG path arcs through the arc SDF path and diagonal line/path strokes through a triangle-body plus round-cap path so macOS-like toolbar glyphs stay continuous without per-pixel dot decomposition. There is no GLFW or toolkit window shim on the macOS desktop path. Empty non-monospace font requests prefer Pretendard before falling back to the system UI face.
...
├── phenotype.state       — Arena, AppState, Scope, InputHandler, scene schedule
...
├── phenotype.widget.glass — Liquid Glass widget style implementations used by the umbrella DSL
