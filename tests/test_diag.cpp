...
void test_snapshot_shape() {
    ...
    assert(system_settings.at("reduce_transparency").is_bool());
    assert(system_settings.at("increase_contrast").is_bool());
    assert(system_settings.at("reduce_motion").is_bool());
    assert(system_settings.at("accessibility_source").is_string());
    ...
    assert(application_runtime.at("render_surface_runtime_owner").as_string()
...
void test_material_runtime_record_json_contract() {
    ...
    assert(group.at("surface_count").as_integer() == 3);
    assert(group.at("union_surfaces").as_integer() == 3);
    assert(group.at("morph_surfaces").as_integer() == 3);
    ...
    assert(group.at("separated_pair_count").as_integer() == 2);
    ...
    assert(container.at("blend_distance").as_float() == 0.0f);
    assert(container.at("interactive").as_bool() == false);
    assert(container.at("requested_morph_transitions").as_bool() == false);
    ...
    assert(container.at("shape_blending_expected").as_bool() == false);
    assert(container.at("reduced_motion_suppressed_morph").as_bool() == false);
    assert(container.at("spacing_clamped").as_bool() == false);
