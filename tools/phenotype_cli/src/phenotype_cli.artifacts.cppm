...
auto verifier_observation_json(VerifierObservation const& verifier)
    ...
    auto const resource_bound_results = verifier.report
        ? material_resource_bound_results_from_report(*verifier.report)
        : std::optional<std::vector<MaterialBudgetBoundResult>>{};
    auto const quality_policy_bound_summary = verifier.report
        ? material_quality_policy_bound_summary_from_report(*verifier.report)
        : std::optional<MaterialBudgetBoundSummary>{};
    auto const quality_policy_bound_results = verifier.report
        ? material_quality_policy_bound_results_from_report(*verifier.report)
        : std::optional<std::vector<MaterialBudgetBoundResult>>{};
    ...
    auto const quality_policy_coverage = verifier.report
        ? material_quality_policy_coverage_from_report(*verifier.report)
        : std::optional<MaterialBoundCoverageSummary>{};
...
void print_material_bound_results(std::string_view label,
    ...
    std::println("{}: {}", label, material_budget_bound_summary_text(*summary));
    for (auto const& line : material_budget_bound_detail_lines(results, summary))
        std::println("  {}", line);
}

void print_verifier_material_resource_bound_summary(
        VerifierObservation const& verifier) {
    if (!verifier.report)
        return;
    print_material_bound_results(
        "material resource bound results",
        material_resource_bound_summary_from_report(*verifier.report),
        material_resource_bound_results_from_report(*verifier.report));
}

void print_verifier_material_quality_policy_bound_summary(
        VerifierObservation const& verifier) {
    if (!verifier.report)
        return;
    print_material_bound_results(
...
void print_verifier_material_bound_coverage(
        VerifierObservation const& verifier,
        std::string_view label,
        std::optional<MaterialBoundCoverageSummary> (*reader)(
            json::Value const&)) {
    if (!verifier.report)
        return;
    auto coverage = reader(*verifier.report);
    if (!coverage)
        return;
...
void print_verifier_material_resource_bound_coverage(
        VerifierObservation const& verifier) {
    print_verifier_material_bound_coverage(
        verifier,
        "material resource bound coverage",
        material_resource_bound_coverage_from_report);
}

void print_verifier_material_quality_policy_coverage(
        VerifierObservation const& verifier) {
    print_verifier_material_bound_coverage(
        verifier,
        "material quality policy coverage",
        material_quality_policy_coverage_from_report);
}
...
void print_verifier_tightest_bound_results(VerifierObservation const& verifier) {
    if (!verifier.report)
        return;
    auto tightest = verifier_tightest_bound_results_text(*verifier.report);
    if (tightest.empty())
        return;
...
void print_verifier_failure_summary(VerifierObservation const& verifier) {
    if (!verifier.report)
        return;
    for (auto const& line : verifier_failure_summary_lines(*verifier.report))
        std::println("{}", line);
}
...
void print_artifact_verify_summary(fs::path const& bundle,
    ...
    print_verifier_material_resource_bound_coverage(verifier);
...
void print_artifact_observation(ArtifactObservation const& observation) {
    ...
    print_verifier_material_resource_bound_coverage(observation.verifier);
