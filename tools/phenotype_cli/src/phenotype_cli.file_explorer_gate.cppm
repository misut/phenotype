...
struct FileExplorerArtifactCase {
    ...
    std::optional<MaterialContainerGroupSummary> material_container_groups;
    std::optional<MaterialBudgetBoundSummary> material_resource_bound_summary;
    std::optional<std::vector<MaterialBudgetBoundResult>>
        material_resource_bound_results;
    std::optional<MaterialBudgetBoundSummary> material_quality_policy_bound_summary;
    std::optional<std::vector<MaterialBudgetBoundResult>>
...
auto file_explorer_case_json(FileExplorerArtifactCase const& item)
    ...
    return std::format(
        ...
        material_container_group_summary_json(item.material_container_groups),
        material_budget_bound_summary_json(item.material_resource_bound_summary),
        material_budget_bound_results_json(item.material_resource_bound_results),
        material_budget_bound_summary_json(
            item.material_quality_policy_bound_summary),
        material_budget_bound_results_json(
...
auto case_label(FileExplorerArtifactCase const& item) -> std::string {
    auto label = item.profile;
    if (!item.mode.empty())
        label += std::format("/{}", item.mode);
    if (!item.scenario.empty() && item.scenario != "default")
        label += std::format("/{}", item.scenario);
    return label;
}

void print_material_budget_summary(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_budget = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return item.material_budget.has_value();
        });
    if (!has_budget)
        return;
...
void print_verifier_manifest_summary(
    ...
    for (auto const& item : cases) {
        ...
        std::println(
            "  {}: name={} runtime-details={} debug-details={} runtime-bounds={} "
...
void print_material_quality_policy_summary(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_quality_policy = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return item.material_quality_policy.has_value();
        });
    if (!has_quality_policy)
        return;

    std::println("material quality policy:");
    for (auto const& item : cases) {
        if (!item.material_quality_policy)
            continue;
        std::println(
            "  {}: {}",
            case_label(item),
            material_quality_policy_text(*item.material_quality_policy));
    }
}
...
void print_material_container_group_summary(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_groups = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return item.material_container_groups.has_value();
        });
    if (!has_groups)
        return;

    std::println("material container groups:");
    for (auto const& item : cases) {
        if (!item.material_container_groups)
            continue;
        std::println(
            "  {}: {}",
            case_label(item),
            material_container_group_summary_text(
                *item.material_container_groups));
    }
}

void print_material_resource_bound_summary(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_bound_summary = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return item.material_resource_bound_summary.has_value();
        });
    if (!has_bound_summary)
        return;

    std::println("material resource bound results:");
    for (auto const& item : cases) {
        if (!item.material_resource_bound_summary)
            continue;
        std::println(
            "  {}: {}",
            case_label(item),
            material_budget_bound_summary_text(
                *item.material_resource_bound_summary));
        for (auto const& line : material_budget_bound_detail_lines(
                 item.material_resource_bound_results,
                 item.material_resource_bound_summary)) {
            std::println("    {}", line);
        }
    }
...
void print_material_quality_policy_bound_summary(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_bound_summary = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return item.material_quality_policy_bound_summary.has_value();
        });
    if (!has_bound_summary)
        return;

    std::println("material quality policy bounds:");
    for (auto const& item : cases) {
        if (!item.material_quality_policy_bound_summary)
            continue;
        std::println(
            "  {}: {}",
            case_label(item),
...
void print_material_tightest_bound_results(
        std::span<FileExplorerArtifactCase const> cases) {
    auto has_tightest = std::ranges::any_of(
        cases,
        [](FileExplorerArtifactCase const& item) {
            return !verifier_tightest_bound_results_text(
                item.verifier_result).empty();
        });
    if (!has_tightest)
        return;

    std::println("material tightest bounds:");
    for (auto const& item : cases) {
        auto tightest = verifier_tightest_bound_results_text(
            item.verifier_result);
        if (tightest.empty())
            continue;
        std::println("  {}: {}", case_label(item), tightest);
    }
}
...
void print_file_explorer_gate(
    ...
    print_material_container_group_summary(summary.cases);
