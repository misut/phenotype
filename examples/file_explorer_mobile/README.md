...
`search-active` startup scenarios through `PHENOTYPE_FILE_EXPLORER_SCENARIO`.
These scenario artifacts keep mobile create, delete, duplicate, navigation
history, preview behavior, sort state, and active search debuggable without
interactive input replay. File
operation scenarios also write an `Operation: ...` receipt into the status
surface so the same shared model contract is visible on mobile artifacts.
