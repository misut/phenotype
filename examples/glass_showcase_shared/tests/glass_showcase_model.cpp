...
int main() {
    ...
    auto material_count =
        demo::parse_glass_expectation("material-count:15");
    ...
    assert(demo::expected_material_plan_count(result.state) == 15);
    assert(demo::expected_material_probe_count(result.state) == 15);
    assert(demo::progress_value(result.state) == 0.85f);
    auto closed_contract = demo::glass_probe_contract(result.state);
    assert(closed_contract.active_material_probe_count == 15);
    assert(closed_contract.total_expected_execution_stages == 58);
    assert(closed_contract.sample_taps_per_probe == 25);
    assert(closed_contract.total_expected_sample_taps == 325);
    ...
    auto selection_probe = demo::glass_material_probe_at(11);
    assert(selection_probe.name == std::string_view{"selection_sidebar_probe"});
    assert(selection_probe.interactive);
    assert(selection_probe.sample_taps == 25u);
    auto outline_probe = demo::glass_material_probe_at(12);
    assert(outline_probe.name == std::string_view{"outline_row_probe"});
    assert(outline_probe.likely_layer
           == std::string_view{"material-outline-row"});
    auto table_header_probe = demo::glass_material_probe_at(13);
    assert(table_header_probe.name == std::string_view{"table_header_probe"});
    assert(table_header_probe.expected_executor == std::string_view{"standard-fill"});
    assert(table_header_probe.sample_taps == 0u);
    assert(!table_header_probe.requires_backdrop_sampling);
    auto disclosure_probe = demo::glass_material_probe_at(14);
    assert(disclosure_probe.name == std::string_view{"disclosure_header_probe"});
    assert(disclosure_probe.stage_count == 3u);
    ...
    assert(demo::expected_material_plan_count(reset_result.state) == 16);
    auto open_contract = demo::glass_probe_contract(reset_result.state);
    assert(open_contract.active_material_probe_count == 16);
    assert(open_contract.total_expected_execution_stages == 62);
    assert(open_contract.total_expected_sample_taps == 350);
    ...
    auto debug_probe = demo::glass_material_probe_at(15);
    ...
    assert(contract["active_material_probe_count"].as_integer() == 16);
    assert(contract["total_expected_execution_stages"].as_integer() == 62);
    assert(contract["total_expected_sample_taps"].as_integer() == 350);
    ...
    assert(probes.contains("selection_sidebar_probe"));
    assert(probes.contains("outline_row_probe"));
    assert(probes.contains("table_header_probe"));
    assert(probes.contains("disclosure_header_probe"));
    assert(probes["visible_blur_probe"].as_object()["kind"].as_string()
           == "thin");
    assert(probes["context_menu_probe"].as_object()["interactive"].as_bool());
    assert(probes["table_header_probe"].as_object()["expected_executor"]
           .as_string() == "standard-fill");
    assert(probes["table_header_probe"].as_object()["sample_taps"].as_integer()
           == 0);
}
