...
Lucide also notes that some icons are derived from Feather Icons under MIT.
Phenotype keeps the Lucide family, icon name, source URL, license URL, and
Apple-asset boundary in `phenotype.icon_catalog` debug metadata so artifacts can
be audited without visual guessing.
